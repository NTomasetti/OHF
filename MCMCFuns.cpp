...
// [[Rcpp::export]]
double nlogDensityVAR (mat data, vec theta, vec mu, mat varInv, int lagsA = 1, int lagsD = 1){
  int T = data.n_rows;
  double dens = 0.5 * log(det(varInv)) - 0.5 * as_scalar((theta - mu).t() * varInv * (theta - mu));
  double rho = 2 / (1 + exp(-theta(2))) - 1;
  int lag = max(lagsA, lagsD);
  for(int t = lag; t < T; ++t){
    double meanA = 0, meanD = 0;
    for(int i = 1; i <= lagsA; ++i){
      meanA += theta(2 + i) * data(t-i, 0)  +  theta(2 + lagsA + i) * abs(data(t-i, 1)) * signDouble(data(t-i, 0));
    } 
    for(int i = 1; i <= lagsD; ++i){
      meanD += theta(2 + 2 * lagsA + i) * data(t - i, 1)  +  theta(2 + 2 * lagsA + lagsD + i) * abs(data(t-i, 0)) * signDouble(data(t-i, 1));
    }
